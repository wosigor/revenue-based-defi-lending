...
contract RevenueBasedLoan is ERC721 {
    ...
    mapping(uint => uint) public idToWithdrawnAmount;
    ...
    function withdrawBorrower(uint _amount) public isActive {
        require(
            msg.sender == borrower,
    ...
    function withdrawLender(uint _amount, uint _loanId) public isActive {
        require(
    ...
    function withdrawLenderDeprecatedLoan(uint _loanId) public {
        require(
            block.timestamp > timeOfCreation + daysToFill * 24 * 3600,
    ...
    function calculateSettledPayout(uint _loanId)
        public
        view
        returns (uint256)
    {
